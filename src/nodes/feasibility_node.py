...
class FeasibilityAnalyzer:
    ...
    def research_technical_requirements(self, startup_idea: str, desirability_analysis: str) -> Dict:
        ...
        try:
            tech_requirements = json.loads(response.content)
        except:
            tech_requirements = {"raw_response": response.content}
    ...
    def build_traction_roadmap(self, startup_idea: str, viability_analysis: str) -> Dict:
        ...
        try:
            ...
            roadmap = {"raw_response": response.content}
    ...
    def create_now_next_later_plan(self, startup_idea: str) -> Dict:
        ...
        print("\n" + "="*80)
        print("NOW, NEXT, LATER ROLLOUT PLAN")
        print("="*80)
        print("""
NOW (0-3 months): PROBLEM/SOLUTION FIT
- Goal: Validate real demand through REVENUE
- Activities: Problem interviews, solution design, demo-sell
- Success: 5-10 paying customers, $500-2K MRR

NEXT (3-24 months): PRODUCT/MARKET FIT
- Goal: Build product customers love, refine based on usage
- Month 3-12: Build & launch MVP
- Month 12-24: Refine & scale to 500+ customers
- Success: PMF signals (NPS 50+, churn <5%, organic >20%)

LATER (24-36 months): SCALE & GROWTH ROCKETS
- Goal: Execute growth rockets for 10x scaling
- Identify best channels: referrals, marketing, partnerships
- Scale to 5K-10K customers, $1M+ MRR
        ...
        try:
            rollout_plan = json.loads(response.content)
        except:
            rollout_plan = {"raw_response": response.content}
